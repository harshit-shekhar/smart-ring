...
MOUSE_UPDATE_MS = 16               
MOVE_SCALE = 0.02                  
GYRO_SMOOTH = 0.85                 # 🔥 INCREASED from 0.70 for buttery smooth gliding
EPS_GYRO = 8.0

# =========================
# 🖱️ CLICK & FREEZE CONFIG
# =========================
IDLE_LOCK_WAIT_MS = 400           
LOCK_DURATION_MS = 400            

TAP_JERK_TRIGGER = 1500.0          
...
# 🔥 INCREASED from 4 to 8. This averages more raw data to kill hardware jitters.
fast_sensor_buf = deque(maxlen=8) 
    ...
    if mouse_active and collecting and fast_sensor_buf:
        ...
        if current_time <= cursor_freeze_time:
            if not click_capture_active and jerk > TAP_JERK_TRIGGER:
                ...
                prev_move = np.array([0.0, 0.0])
                cursor_freeze_time = current_time + ML_CAPTURE_WINDOW_MS + 200 # Extend shield
